...
class ai:
	...
	def evaluationFunctionRL(self, selfCharacter, switchWeapons, pos, target):
	...
	def calculateMoveRL(self, character):
		if not character.moveable():
			return None
		...
		#print character.getName()
		#print (bestPosition, bestSwitchWeapons, bestTargetAttcked)
		return (bestPosition, bestSwitchWeapons, bestTargetAttcked, bestScore)


	def randomMoveRL(self, character):
		if not character.moveable():
			return None
		...
		numEndPositions = len(endPositions)
		randomIndex = random.randrange(0, numEndPositions)
		endPos = endPositions[randomIndex]
		...
		targets = self.gameboard.getTargets(character, endPos)
		...
		if numTargets != 0:
			randIndex = random.randrange(0, numTargets)
		...
		if target != None:
			...
			if character.isEnemy():
				primWeaponScore = self.enemyEvaluationFunction(character, endPos, target)
			else:
				primWeaponScore = self.evaluationFunction(character, False, endPos, target)
			...
			if secWeapon != None:
				if character.isEnemy():
					secWeaponScore = self.enemyEvaluationFunction(character, endPos, target)
				else:
					secWeaponScore = self.evaluationFunction(character, True, endPos, target)
		...
		return (endPos, switchWeapons, target, score)
